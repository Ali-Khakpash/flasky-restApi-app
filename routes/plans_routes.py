...
def update_book_detail(id):
     ...
     if not authorize.update(plan_obj) or not authorize.has_role('ADMIN_WITH_UPDATE_PRIVILEGE'):
         raise Unauthorized
...
@plans_routes.route('category', methods=['POST','GET'])
@jwt_required
...
@plans_routes.route('plans/authorize', methods=['POST'])
@jwt_required
@authorize.create(Plan)
def create_plan_authorize():
    # data = request.get_json()
    # new_plan = Plan(data['title'], data['short_desc'])
    # db.session.add(new_plan)
    # db.session.commit()
    return jsonify(msg='Created Plan', curr_user = current_user.username), 200


@plans_routes.route('plans/<id>', methods=['GET'])
@jwt_required
def update_the_plan(id):
    plan = Plan.query.get(id)
    if not authorize.has_role('ADMIN_WITH_UPDATE_PRIVILEGE') :
        raise Unauthorized
    return
